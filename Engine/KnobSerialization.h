...
createDefaultValueForParam(const std::string& paramName,
                           ...
                           const T& value1)
{
    std::shared_ptr< Knob<T> > knob = std::make_shared<Knob<T> >(nullptr, paramName, 2, false);
...
createDefaultValueForParam(const std::string& paramName,
                           const T& value)
{
    std::shared_ptr< Knob<T> > knob = std::make_shared<Knob<T> >(nullptr, paramName, 1, false);
        ...
        , _isOpened(false)
    {
        ...
        for (U32 i = 0; i < children.size(); ++i) {
            ...
            if (isGrp) {
                ...
                //bool copyKnob = false;//isChoice != nullptr;
